...
const OUT = "/Users/yuza/chess-agent-v2/Sentio_AI_Features_Prolog_Light_Ember.pptx";
const BUILD = "/Users/yuza/chess-agent-v2/.presentation-light";
const ASSETS = `${BUILD}/assets`;
const W = 1280, H = 720;
const C = {
  bg: "#F7F4EE", panel: "#FFFDF9", panel2: "#E9E3D9", ink: "#211D18",
  muted: "#746B60", amber: "#D97706", amber2: "#B45309", cream: "#40382F",
  olive: "#779556", cyan: "#22D3EE", blue: "#38BDF8", green: "#34D399",
  red: "#DC4C4C", orange: "#EA7A24", purple: "#7C5CCB", line: "#D8CEC0",
};
...
function coverDevIssueBadge(slide, x, y, w, h, fill) {
  shape(slide, "rect", x, y, w, h, fill);
}

// 1 — cover
{
  const s = p.slides.add(); s.background.fill = C.bg;
  shape(s, "ellipse", 820, 0, 460, 460, "#F3DFC1");
  shape(s, "ellipse", 900, 300, 380, 380, "#DCEAF0");
...
// 10 worked example
{
  ...
  shape(s,"roundRect",690,200,530,410,"#FFF4E0",C.amber,"rounded-xl");
...
// 12 agents screenshot
{
  ...
  await imageOn(s,"sentio-light-3d.png",60,185,710,430,"cover","Sentio 3D arena in light mode with AI decision controls");
  coverDevIssueBadge(s,60,579,82,36,"#182432");
  ...
  notes(s,["lib/agents.ts","lib/minimax.ts","lib/mcts.ts","components/AgentsGraph3D.tsx","Local light-mode screenshot: .presentation-light/assets/sentio-light-3d.png"],"The four agents are an analysis and visualization feature. They expose tradeoffs rather than replacing the production Stockfish opponent.");
}

// 13 coach screenshot
{
  const s=p.slides.add(); base(s,13,"SENTIO / LANGUAGE AI"); title(s,"The coach grounds natural language in the live position");
  await imageOn(s,"sentio-light-main.png",55,180,730,440,"cover","Sentio AI Coach and Burmese voice coach interface in light mode");
  coverDevIssueBadge(s,55,584,86,36,"#EFF2F7");
  ...
  notes(s,["app/api/coach/route.ts","lib/coachPrompt.ts","components/AIAnalysisTab.tsx","Local light-mode screenshot: .presentation-light/assets/sentio-light-main.png"],"Explain graceful fallback: the route builds structured advice first, then enhances it with an LLM when available.");
}
...
// 15 product
{
  ...
  await imageOn(s,"sentio-light-analysis.png",45,168,850,480,"cover","Sentio analysis workspace with chessboard and controller in light mode");
  coverDevIssueBadge(s,45,607,104,41,"#EFF2F7");
  ...
  notes(s,["components/BoardWorkspace.tsx","components/ControllerPanel.tsx","components/EmotionMonitor.tsx","Local light-mode screenshot: .presentation-light/assets/sentio-light-analysis.png"],"Show that the adaptive system exposes its current state: emotion, engine profile, evaluation, and analysis tools.");
}
...
// 18 close
{
  ...
  shape(s,"roundRect",840,105,340,170,"#FFF4E0",C.amber,"rounded-xl");
