...
export default function BenchmarkTab({ report }: BenchmarkTabProps) {
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-3 overflow-y-auto pr-1 chat-scroll">
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="rounded-full border border-emerald-400/30 bg-emerald-400/10 px-2 py-0.5 text-[10px] font-bold text-emerald-200 light:text-emerald-700">LOCAL RUN</span>
            </div>
      ...
      <div className="flex flex-wrap gap-2 rounded-xl border border-zinc-800/80 bg-zinc-950/50 p-3 light:border-slate-300 light:bg-white/70">
        <label className="flex min-w-[150px] flex-1 flex-col gap-1 text-[10px] uppercase tracking-wider text-zinc-500">
          Position
          <select value={position} onChange={(event) => setPosition(event.target.value)} className="rounded-md border border-zinc-700 bg-zinc-900 px-2 py-1.5 text-xs normal-case tracking-normal text-zinc-200 outline-none light:border-slate-300 light:bg-white light:text-slate-800">
            {positions.map((option) => <option key={option} value={option}>{option === "all" ? "All positions" : option}</option>)}
          </select>
        </label>
        <label className="flex min-w-[180px] flex-1 flex-col gap-1 text-[10px] uppercase tracking-wider text-zinc-500">
          Metric
          <select value={metric} onChange={(event) => setMetric(event.target.value as MetricKey)} className="rounded-md border border-zinc-700 bg-zinc-900 px-2 py-1.5 text-xs normal-case tracking-normal text-zinc-200 outline-none light:border-slate-300 light:bg-white light:text-slate-800">
