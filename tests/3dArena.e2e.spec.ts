...
test.describe("3D arena", () => {
  ...
  test("hides a captured piece during robot choreography and restores the final board", async ({ page }) => {
    ...
    await expect.poll(async () => (await snapshot(page)).robotCaptureHidden, { timeout: 5_000 }).toBe(true);
    await expect.poll(async () => (await snapshot(page)).robotAnimating, { timeout: 20_000 }).toBe(false);
